...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const { data, count, error: err } = await db.from('faqs').select('*', { count: 'exact' })
    ...
    const { data, error: err } = await db.from('faqs').update(updates).eq('id', id).select().single();
    if (err) return error(res, err.message, 500);
    ...
    const { id } = query;
    if (!id) return error(res, 'Missing id', 400);
    const { error: err } = await db.from('faqs').delete().eq('id', id);
    if (err) return error(res, err.message, 500);
