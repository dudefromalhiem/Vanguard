...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const { data, count, error: err } = await db.from('projects').select('*', { count: 'exact' })
      .order('created_at', { ascending: false }).range(offset, offset + limit - 1);
    if (err) return error(res, err.message, 500);
    return success(res, { data, count, limit, offset });
  } else if (req.method === 'POST') {
    const body = await parseBody(req);
    if (!validateRequired(body, ['title', 'description'])) return error(res, 'Missing required fields', 400);
    if (!body.slug) body.slug = slugify(body.title);
    const { data, error: err } = await db.from('projects').insert(body).select().single();
    if (err) return error(res, err.message, 500);
    ...
    const { data, error: err } = await db.from('projects').update(updates).eq('id', id).select().single();
    if (err) return error(res, err.message, 500);
